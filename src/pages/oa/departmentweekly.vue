  ...
  methods: {
    ...
    go() {
      if (this.userArr && this.userArr !== "") {
        setItem("weeklySendTo", this.userArr);
        // setItem("sendToId", this.userArr);
        // console.log(this.$route.query)
        // console.log(this.radioType)
        // let approverId = this.$route.query
        this.$router.push({ path: "/oaSystem/weekly" });
        // this.$router.push({path: '/oaSystem/leave',query:{approverId:approverId,sendTo:this.radioType}})
      }
    },
    change($event,val) {
      // if(e != this.userArr[0]){
