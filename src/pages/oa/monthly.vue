  ...
  <div class="leave">
      ...
      <div class="dayly_contain">
          <el-row class="leave_name gray_line">
              <el-col :span="8">本月完成工作</el-col>
              <el-col :span="16">
                  <input type="text" class="edit_input" placeholder="请填写本月完成工作" v-model="done"/>
                  <i class="el-icon-circle-close" @click="removeDone"></i>
              </el-col>
          </el-row>
          <el-row class="leave_name">
              <el-col :span="8">本月工作总结</el-col>
              <el-col :span="16">
                  <input type="text" class="edit_input" placeholder="请填写本月工作总结"  v-model="summary"/>
                  <i class="el-icon-circle-close" @click="removeSummary"></i>
              </el-col>
          </el-row>
          <el-row class="leave_name">
              <el-col :span="8">下月工作计划</el-col>
              <el-col :span="16">
                  <input type="text" class="edit_input" placeholder="请填写下月工作计划" v-model="workPlan"/>
          ...
          <el-row class="leave_name gray_line">
            ...
            <el-col :span="24" class="textarea">
                <ul class="nameList">
                    <li class="userName"><span>{{monthlyApprover}}</span></li>
                </ul>
            ...
            <el-col :span="24" class="textarea">
               <ul class="nameList">
                    <li class="userName"><span v-for="(item,i) in monthlySendtoinfo" :key="i">{{item}}</span></li>
                </ul>
...
export default {
  data() {
    return {
      monthlySendtoinfo: JSON.parse(getItem("monthlySendTo")),
      monthlyApprover: getItem("monthlyApprover"),
      done: "",
      summary: "",
      workPlan: "",
      remark: "",
  ...
  methods: {
    initData() {
      ...
      let monthlyInfo = getItem("monthly");
      if (monthlyInfo) {
        getItem("monthly");
        let monthlyInfo = JSON.parse(localStorage.monthly);
        console.log(monthlyInfo);

        // console.log(recordInfo.type)
        this.done = monthlyInfo.done;
        this.summary = monthlyInfo.summary;
        this.workPlan = monthlyInfo.workPlan;
        this.remark = monthlyInfo.remark;
      }
    },
    goApprover() {
      setItem("monthly", {
      ...
      this.$router.push("/oaSystem/personnelmonthly");
    },
    sendTo() {
      setItem("monthly", {
        done: this.done,
      ...
      this.$router.push("/oaSystem/departmentmonthly");
    },
    ...
    async submit() {
      getItem("userinfo");
      getItem("monthlyApprover");
      getItem("monthlyApproverId");
      getItem("monthlySendto");
      ...
      var data = {
        ...
        approver: localStorage.monthlyApproverId,
        sendTo: this.monthlySendtoinfo
      };
       var enData = strEnc(JSON.stringify(data), "ZND20171030APIMM" );
      if (!this.done.trim()) {
        this.$message("本月完成不能为空");
      } else if (!this.summary.trim()) {
        this.$message("本月总结不能为空");
      } else if (!this.workPlan.trim()) {
        this.$message("下月工作计划不能为空");
      } else if (!this.remark.trim()) {
        this.$message("备注信息不能为空");
      } else if (!localStorage.monthlyApprover) {
        this.$message("发送至不能为空");
      } else {
        let res = await oaMonthly(enData);
